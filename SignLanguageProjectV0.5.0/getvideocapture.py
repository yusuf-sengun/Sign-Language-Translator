...
class getVideoCapture:
    def __init__(self):
        #paths
        datasetPath=os.getcwd()+r"\Resources\dataset.png"
        ...
        while(self.cap.isOpened()):
           ...
           datasetImage=cv2.imread(datasetPath)
           cv2.imshow('Dataset',datasetImage)
           cv2.imshow("Image",self.img)
           key=cv2.waitKey(33)
           if key == ord(quitKey):
               cv2.destroyAllWindows()
               break
           elif key==ord(predictKey):
               spellWord=self.preProcessDidYouMean(self.prediction.myString)
               spelledWord=didYouMean.correction(spellWord,self.img)
               self.changeWord(spelledWord)
               print(self.prediction.myString)
           elif key==ord(backspaceKey):
              self.prediction.myString=self.prediction.myString[:-1]
           elif key==ord(spaceKey):
